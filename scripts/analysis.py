...
# this is currently not used as well
...
# this function goes through the dictionary and reorganize it so that the keys are the tuple of (timestamp, main device)
def sort_data_timestamp_and_device(dict):
    sd = SortedDict()
    for key in dict:
        for index in range(len(dict[key])):
            if len(dict[key][index]) == 3:
                sd.setdefault((int(dict[key][index][0]), key[1]), []).append((key[0], dict[key][index][1], False, -1))
            elif len(dict[key][index]) == 4:
                sd.setdefault((int(dict[key][index][0]), key[1]), []).append((key[0], dict[key][index][1], True, dict[key][index][3]))
    return sd
...
def merge_dataframe(motion1, motion2, motion3, prox_data):
    ...
    dfMotion03 = pd.DataFrame(list(motion1.items()), columns=['key', 'motion03'])
    dfMotion06 = pd.DataFrame(list(motion2.items()), columns=['key', 'motion06'])
    dfMotion07 = pd.DataFrame(list(motion3.items()), columns=['key', 'motion07'])
    df = pd.DataFrame(list(prox_data.items()), columns=['key', 'data'])
    df_merge = pd.merge(df, dfMotion07, on=['key'])
    # merge the data frames on timestamps
    df_merge = pd.merge(df_merge, dfMotion06, on=['key'])
    df_merge = pd.merge(df_merge, dfMotion03, on=['key'])
    # write to output file
    tfile = open('Merged.txt', 'w+')
    tfile.write(df_merge.to_string())
    tfile.close()


# first process log file by calling the two process functions
mDict = process_single2(process_single1("03_SS_20210109.LOG"))
...
motion03 = motion_analysis("motion_file_03.csv")
motion06 = motion_analysis("motion_file_06.csv")
motion07 = motion_analysis("motion_file_07.csv")

# finally call the merge function to merge all the data
merge_dataframe(motion03, motion06, motion07, sd)