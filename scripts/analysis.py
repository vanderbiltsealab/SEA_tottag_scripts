...
ZERO_RANGE_CODE = 000000
# DATAFILE = "03_SS_20210109.LOG"
inRangeDist = 915  # 3ft = 915mm

total_args = len(sys.argv)

start_logfile_index_in_argv = 1
end_logfile_index_in_argv = total_args - 1 # -1 because in range() function
# does not include last value
# argv[0] == analysis.py
# argv[total_args - 1]  == x
x_value = total_args - 1

# argv is the array of command line arguments after 'python3' starting at index 0

for logfile in range(start_logfile_index_in_argv, end_logfile_index_in_argv):
    print()
    DATAFILE = sys.argv[logfile]

    x = int(sys.argv[x_value]) # x value represents window of seconds in Step1-3
#    x = int(sys.argv[2])
    # ************************************************************************ #
    ...
    with open(DATAFILE) as f:
        ...
        for line in f:
            ...
            if line[0] != '#':
                ...
                if ((int(tokens[2]) != OUT_OF_RANGE_CODE) & (int(tokens[2]) != OUT_OF_RANGE_CODE)):
    ...
    for key in filtered:
        for index in range(len(filtered[key])):
            ...
            print(str(filtered[key][index][0]) + "\t" + str(key) + "\t" + str("{:10.9f}".format(dist)) + "\t\t1\t\t\t" + str(index+1))
